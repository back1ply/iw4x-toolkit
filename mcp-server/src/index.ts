...
/** Maximum matches iwd_grep returns before truncating, to prevent context flood. */
const GREP_MAX_MATCHES_DEFAULT = 50;
...
export function globToRegex(pattern: string): RegExp {
  // Tokenize first so escaping literal chars doesn't clobber the wildcards.
  // Split on wildcards, preserving them as tokens.
...
/**
 * Build a ±ctx-line diff snippet around a changed line.
 *
 * - If `hintLine` is supplied (0-indexed), the context window is centred there.
 *   This lets callers that know the exact replacement offset (e.g. iwd_patch) show
 *   the right section, even when the very first diff line is not the best anchor.
 * - Falls back to scanning for the first differing line when hintLine is omitted.
 * - Returns an empty snippet when the two strings are identical.
 */
export function buildDiffSnippet(
  ...
  // Determine anchor: use hintLine if given, otherwise scan for first difference
...
/**
 * Open and validate an IWD (ZIP) file. Returns the AdmZip instance or an error message.
 */
function openIwd(resolved: string): { zip: AdmZip } | { error: string } {
...
/** Shared entry-not-found error — avoids repeating the same message across tools. */
function entryNotFoundErr(normalized: string) {
  return errResult(
...
server.tool(
  ...
  async ({ path: iwdPath, pattern, summary_only, names_only }) => {
    ...
    // summary_only: one-line extension breakdown, ignores pattern
    if (summary_only) {
...
server.tool(
  ...
  async ({ path: iwdPath, entry, content, dry_run }) => {
    ...
    // If replacing an existing text entry, capture old content for diff
    let diffSection = "";
...
server.tool(
  ...
  async ({ path: iwdPath, entry, old: oldStr, new: newStr, count, dry_run }) => {
    ...
    // Perform replacement and track the line of the first replacement for the diff hint
    ...
    // Pass hintLine so the snippet is centred on the actual replacement, not line 0
    ...
    const diffBlock =
      `--- Context around change (line ${changedLine + 1}) ---\n${snippet}`;
...
server.tool(
  ...
  async ({ path: iwdPath, pattern, entry_glob, is_regex, max_matches }) => {
    ...
    // Validate regex up front so the error is clear
    if (is_regex) {
      try {
        new RegExp(pattern);
      } catch (e) {
    ...
    if (searchEntries.length === 0) {
      return okResult(
        entry_glob
          ? `No text entries match glob '${entry_glob}' in ${resolved}.\nTip: use iwd_list to check available entry names.`
          : `No text entries found in ${resolved}.`,
      );
    }
    ...
    outer: for (const entry of searchEntries) {
      ...
      for (let idx = 0; idx < lines.length; idx++) {
        ...
        const matched = is_regex
          ? (searcher as RegExp).test(line)
          : line.toLowerCase().includes(searcher as string);
        if (matched) {
          matchesInFile.push(`  ${idx + 1}: ${line.trim()}`);
          totalMatches++;
          if (totalMatches >= max_matches) {
            truncated = true;
            break; // stop collecting immediately — respect cap strictly
          }
        }
      }
    ...
    return okResult(
      ...
      (entry_glob
        ? ` (filtered to '${entry_glob}', ${searchEntries.length} entries searched)`
        : ` (${searchEntries.length} text entries searched)`),
    );
  },
);
  ...
  async ({ path: iwdPath, dest, entry_glob, dry_run }) => {
    ...
    for (const entry of entries) {
      ...
      try {
        ...
        // Clean up any files already written before failing, to avoid partial extraction
        for (const written of extracted) {
          try { fs.unlinkSync(path.join(destResolved, written)); } catch { /* best-effort */ }
        }
        return errResult(
...
server.tool(
  ...
  async ({ src_path, src_entry, dst_path, dst_entry, overwrite, dry_run }) => {
    ...
    // Open dest (may be the same file)
...
// ---------------------------------------------------------------------------
// Exports (for testing) — functions are exported inline above; server re-exported here
// ---------------------------------------------------------------------------
