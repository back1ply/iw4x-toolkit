...
// --- Tool: iwd_list ---
server.tool("iwd_list", "List all entries in an IWD file (name, size, compressed size)", { path: z.string().describe("Path to the IWD file") }, async ({ path: iwdPath }) => {
...
// --- Tool: iwd_read ---
server.tool("iwd_read", "Read a file from inside an IWD archive. Returns UTF-8 text for text files, base64 for binary files.", {
...
// --- Tool: iwd_write ---
server.tool("iwd_write", "Write or update a file inside an IWD archive. Creates a .bak backup on first modification per session. Content is UTF-8 text.", {
    path: z.string().describe("Path to the IWD file"),
...
// --- Tool: iwd_remove ---
server.tool("iwd_remove", "Remove a file from inside an IWD archive. Creates a .bak backup on first modification per session.", {
    path: z.string().describe("Path to the IWD file"),
...
// --- Tool: iwd_diff ---
server.tool("iwd_diff", "Compare two IWD files and report added, removed, modified (CRC mismatch), and identical entries.", {
...
// --- Resource: iw4x://dvars ---
server.resource("DVAR Reference", "iw4x://dvars", { description: "MW2/IW4X DVAR knowledge base — 700+ DVARs with types, defaults, flags, categories, and FPS impact" }, async () => ({
    contents: [
        {
            uri: "iw4x://dvars",
